...
class base {
    ...
    loginPromisify = this.Promisify(this.wxlogin)  // 小程序登陆
    /**
    ...
    */
    ...
    ajaxQueue(obj={}){
        return this.login().then(res=>{
            ...
            return  this.ajaxQueueRequest(obj)
        })
    }
    // 第一次 写 若出错 纯属 正常操作 
    // 当你需要 防止 出现双击  请求发送两遍  这种场景的时候
    ajaxQueueRequest (obj={}){//ajax 请求promise化
        //发起  请求
        //嵌套promise
        var _this=this;
        return new Promise((resolve,reject)=>{
            ...
            if(obj.method==='GET'){  // 当是get 请求的时候 把参数 放在最后
                obj.url+'?'+str.slice(0,-1);
            ...
            setTimeout(() => {
                //若队列里面  有长度
                if(_this.queue.length&&!_this.queueFlag){
                    //console.log(123)
    ...
    nextTick(){
        if(this.queue.length){  // 进行下一个队列
            let nextQueue=_this.queue.shift();
